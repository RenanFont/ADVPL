...
@type function
/*/
user function RELDOCENT()
	Local dDataDe	:= Date()
	Local dDataAte	:= Date()
	Local lRet 		:= .T.
	Local aParamBox	:= {}
	Private cTitulo 	:= "Relatório Relação NF Entrada Fornecedor UNION"
...
Static Function ReportDef(cAlias, cGrupo)
	...
	Local cTitle  := "Relatório Relação NF Entrada Fornecedor UNION"
	Local cHelp   := "Relatório Relação NF Entrada Fornecedor UNION"
	Local oReport
	Local oSection1

	oReport := TReport():New('RELDOCENTR',cTitle,,{|oReport| RptPrtV3(oReport,cAlias)},cHelp)
	...
	TRCell():New(oSection1,"CODIGO"						,	cAlias, "CODIGO", "", 6,,								{ || (cAlias)->A2_COD } )
	TRCell():New(oSection1,"FORNECE"					,	cAlias, "FORNECE", "", TamSX3("A2_NOME")[1],,			{ || (cAlias)->FORNECE } )
...
Static Function RptPrtV3( oReport, cAlias )
	...
	cQuery += " 	CAB.XML_EMIT CNPJ,A2_COD,UPPER(XML_NOMEMT) FORNECE, A2_EST UF,XIT_CFNFE CFOP_CLI,                                                                             " + CRLF
	...
	cQuery += " 	LEFT JOIN SA2010 A2 (NOLOCK) ON A2_CGC = CAB.XML_EMIT AND A2.D_E_L_E_T_ = ''                                                                                  " + CRLF
