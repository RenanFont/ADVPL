...
@version 1.0
/*/
wsRestful WsEspecieOFM DESCRIPTION "Integracao da especie de DOC - Protheus x OFM"
	wsData empresa 		As String OPTIONAL
	wsData cod 			As String OPTIONAL		
	wsData descri		As String OPTIONAL

	wsMethod GET	Description "Retorna os dados da rota de WsEspecieOFM"	wsSyntax "/WsEspecieOFM/get"
	wsMethod POST	Description "Post WsEspecieOFM"							wsSyntax "/WsEspecieOFM/post"
end wsRestful

wsMethod GET wsReceive empresa, cod, descri  wsService WsEspecieOFM	
	local oJson 		:= JsonObject():New()
	local nSeconds		:= Seconds()
	local cError		:= ""
	local cFilParam		:= GetMv("ZZ_FLPROFM", .F., "01")
	local oEntidaOFM	:= EspecieOFM():new_EspecieOFM()
	local lValSeg		:= .F.
...
wsMethod POST wsService WsEspecieOFM	
return (.T.)
