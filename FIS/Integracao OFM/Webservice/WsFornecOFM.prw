...
@version 1.0
/*/
wsRestful WsFornecOFM DESCRIPTION "Integracao de fornecedor Protheus x OFM"
	...
	wsMethod GET Description "Retorna os dados da rota de WsFornecOFM" wsSyntax "/WsFornecOFM/get"
	wsMethod POST Description "Post WsFornecOFM" wsSyntax "/WsFornecOFM/post"
end wsRestful

wsMethod GET wsReceive empresa, nome, razao, cod, loja, cnpj, sincroniza  wsService WsFornecOFM	
	local oJson 		:= JsonObject():New()
	local nSeconds		:= Seconds()
	local cError		:= ""
	local cFilParam		:= GetMv("ZZ_FLPROFM", .F., "01")
	local oEntidaOFM	:= FornecOFM():new_FornecOFM()
	local lValSeg		:= .F.
...
wsMethod POST wsService WsFornecOFM	
return (.T.)
