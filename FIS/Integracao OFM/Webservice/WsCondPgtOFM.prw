...
/*/{Protheus.doc} WsCondPgtOFM
Tela para consulta dos Condicao Pagto Protheus x OFM

@author Fabio Hayama - Geeker Company
@since 04/12/2016
@version 1.0
/*/
wsRestful WsCondPgtOFM DESCRIPTION "Integracao de Condicao Pagto Protheus x OFM"
	...
	wsMethod GET	Description "Retorna os dados da rota de WsCondPgtOFM"	wsSyntax "/WsCondPgtOFM/get"
	wsMethod POST	Description "Post WsCondPgtOFM"							wsSyntax "/WsCondPgtOFM/post"
end wsRestful

wsMethod GET wsReceive empresa, cod, descri, sincroniza wsService WsCondPgtOFM	
	local oJson 		:= JsonObject():New()
	local nSeconds		:= Seconds()
	local cError		:= ""
	local cFilParam		:= GetMv("ZZ_FLPROFM", .F., "01")
	local oEntidaOFM	:= CondPgtOFM():new_CondPgtOFM()
	local lValSeg		:= .F.
...
wsMethod POST wsService WsCondPgtOFM	
return (.T.)
