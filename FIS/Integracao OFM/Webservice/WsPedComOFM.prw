...
@version 1.0
/*/
wsRestful WsPedComOFM DESCRIPTION "Integracao de pedido de compras Protheus x OFM"
	...
	wsData numped 		As String OPTIONAL
	wsData emissao 		As String OPTIONAL
	wsData sincroniza	As String OPTIONAL
		
	wsMethod GET Description "Retorna os dados da rota de WsPedComOFM" wsSyntax "/WsPedComOFM/get"
	wsMethod POST Description "Post WsPedComOFM" wsSyntax "/WsPedComOFM/post"
end wsRestful

wsMethod GET wsReceive empresa, cgcfor, fornec, loja, nomefor, razaofor, numped, emissao, sincroniza wsService WsPedComOFM	
	local oJson 		:= JsonObject():New()
	local nSeconds		:= Seconds()
	local cError		:= ""
	local cFilParam		:= GetMv("ZZ_FLPROFM", .F., "01")
	local oEntidaOFM	:= PedComOFM():new_PedComOFM()
	local lValSeg		:= .F.
...
wsMethod POST wsService WsPedComOFM	
return (.T.)
