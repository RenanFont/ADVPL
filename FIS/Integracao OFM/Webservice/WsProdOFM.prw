...
/*/{Protheus.doc} WsProdOFM
Tela para consulta dos Produto Protheus x OFM

@author Fabio Hayama - Geeker Company
@since 04/12/2016
@version 1.0
/*/
wsRestful WsProdOFM DESCRIPTION "Integracao de Produto Protheus x OFM"
	...
	wsMethod GET Description "Retorna os dados da rota de WsProdOFM" wsSyntax "/WsProdOFM/get"
	wsMethod POST Description "Post WsProdOFM" wsSyntax "/WsProdOFM/post"
end wsRestful

wsMethod GET wsReceive empresa, cod, descri, sincroniza wsService WsProdOFM	
	local oJson 		:= JsonObject():New()
	local nSeconds		:= Seconds()
	local cError		:= ""
	local cFilParam		:= GetMv("ZZ_FLPROFM", .F., "01")
	local oEntidaOFM	:= ProdOFM():new_ProdOFM()
	local lValSeg		:= .F.
...
wsMethod POST wsService WsProdOFM	
return (.T.)
