...
/*/{Protheus.doc} WsTranspOFM
Tela para consulta das transportadoras Protheus x OFM

@author Fabio Hayama - Geeker Company
@since 04/12/2016
@version 1.0
/*/
wsRestful WsTranspOFM DESCRIPTION "Integracao de transportadoras Protheus x OFM"
	...
	wsMethod GET Description "Retorna os dados da rota de WsTranspOFM" wsSyntax "/WsTranspOFM/get"
	wsMethod POST Description "Post WsTranspOFM" wsSyntax "/WsTranspOFM/post"
end wsRestful

wsMethod GET wsReceive empresa, nome, cod, cnpj, sincroniza  wsService WsTranspOFM	
	local oJson 		:= JsonObject():New()
	local nSeconds		:= Seconds()
	local cError		:= ""
	local cFilParam		:= GetMv("ZZ_FLPROFM", .F., "01")
	local oEntidaOFM	:= TranspOFM():new_TranspOFM()
	local lValSeg		:= .F.
...
wsMethod POST wsService WsTranspOFM	
return (.T.)
