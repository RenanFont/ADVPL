...
/*/{Protheus.doc} WsNaturOFM
Tela para consulta dos Natureza Protheus x OFM

@author Fabio Hayama - Geeker Company
@since 04/12/2016
@version 1.0
/*/
wsRestful WsNaturOFM DESCRIPTION "Integracao de Natureza Protheus x OFM"
	...
	wsMethod GET	Description "Retorna os dados da rota de WsNaturOFM"	wsSyntax "/WsNaturOFM/get"
	wsMethod POST	Description "Post WsNaturOFM"							wsSyntax "/WsNaturOFM/post"
end wsRestful

wsMethod GET wsReceive empresa, cod, descri, sincroniza wsService WsNaturOFM	
	local oJson 		:= JsonObject():New()
	local nSeconds		:= Seconds()
	local cError		:= ""
	local cFilParam		:= GetMv("ZZ_FLPROFM", .F., "01")
	local oEntidaOFM	:= NaturOFM():new_NaturOFM()
	local lValSeg		:= .F.
...
wsMethod POST wsService WsNaturOFM	
return (.T.)
